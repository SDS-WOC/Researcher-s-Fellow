...
<body>
 <h1>Projects</h1>
  <?php 
     include 'conn.php';
     //$table1="create table manojmishra_project (projectname varchar(255),projectdesc varchar(255))";
     //$instable1=$conn->query($table1);
     //$table2="create table manojmishra_intern  (projectname varchar(255),interndesc varchar(255))";
   ...
   echo "<script>alert('Incorrect Email or Password');</script>";
          ...
          $aboutintern=$_POST["aboutintern"];
            ...
            echo "<script> alert('Successfully Posted');</script>";
             ...
             $variable=$_POST['name'];
             $del="delete from manojmishra_intern where projectname ='$variable'";
             $final=$conn->query($del);
           }
           }
          if(!empty($projname) && !empty($aboutproj!=null)){
                  $insrt_project="insert into manojmishra_project values ('$projname','$aboutproj')";
                  $check_project=$conn->query($insrt_project);
                 }
                 if(!empty($projname) && !empty($aboutintern)){
                  $insrt_intern="insert into manojmishra_intern values ('$projname','$aboutintern')";
...
<?php 
    $display_project="select* from manojmishra_project";
    $display_table_project=$conn->query($display_project);
      ...
      <div id="right">
        <h1>Intern Needed</h1>
<?php 
    $display_intern="select* from manojmishra_intern";
